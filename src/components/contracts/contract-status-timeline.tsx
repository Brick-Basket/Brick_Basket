...
import { CONTRACT_STATUS_STEPS } from "@/components/contracts/contract-status-config";
import type { ContractStatus } from "@/types/domain/contract";

const STEP_INDEX: Record<ContractStatus, number> = {
  draft: 0,
  sent_for_acceptance: 1,
  accepted: 2,
  declined: 2,
};

/** Horizontal status timeline required by the Contract Management module. */
export function ContractStatusTimeline({ status }: { status: ContractStatus }) {
  ...
  return (
    <ol className="flex items-center gap-2">
      {CONTRACT_STATUS_STEPS.map((step, index) => {
        ...
        return (
          <li key={step.key} className="flex flex-1 items-center gap-2">
            ...
            {index < CONTRACT_STATUS_STEPS.length - 1 && (
